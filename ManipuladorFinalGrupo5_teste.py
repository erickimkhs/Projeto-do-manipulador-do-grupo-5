...
def traj_solda_reta(p0,pf,vel_solda):

    T = np.linalg.norm(pf - p0)/vel_solda
    N = max(int(T/0.05),20)

    traj = np.zeros((N,3))

    for i in range(3):
        traj[:,i] = np.linspace(p0[i],pf[i],N)
...
while True:
    ...
    if opcao == 0:
        ...
        print('\nDigite o ponto inicial e final da solda:')
        try:
            P0 = np.array([float(x) for x in input('P0 [x,y,z]: ').replace('[','').replace(']','').split(',')])
            Pf = np.array([float(x) for x in input('Pf [x,y,z]: ').replace('[','').replace(']','').split(',')])
        except:
            print('Erro na entrada dos pontos da solda.')
            continue

        print('\nDigite o ângulo da solda (graus) e a velocidade da solda (mm/s):')
        try:
            ang_solda = float(input('Ângulo da Solda: '))
            vel_solda = float(input('Velocidade da Solda: '))
        ...
        # Gerar trajetória
        vel = vel_solda/1000
        traj,N = traj_solda_reta(P0,Pf,vel)

        # Calculo de az
        az = np.cos(np.deg2rad(ang_solda))

        print('\nExecutando solda reta...')
        print(f'\nVelocidade: {vel_solda:.1f} mm/s')
